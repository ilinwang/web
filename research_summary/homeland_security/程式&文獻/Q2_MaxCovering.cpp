 /*
	Limited guard numbers, find the best patroling route 
...
int **addStopNum; //for crossnode
int **addStopIndex; // for crossnode
int *nodeNum = new int [trackN];// how many nodes in each track
int *arcNum = new int [trackN];
int crossArcNum = 0;
vector<vector<SameNode> > sameNode;
vector<vector<vector<int> > > stopNode;// which nodes belongs to this stop 
vector<vector<vector<int> > > trainNode;//which nodes belongs to this train
...
int main(){
    ...
    for(int i=0; i<trackN; i++){
        ...
        for(int j=0; j<crossNode.size(); j++){
            addStopNum[i][j] = 0;// counting how many stops
   ...
   //cout<<"nodeNum: "<<nodeNum[n]<<endl;
	...
	for(int i=0; i<stopNode[n].size();i++){ // for each stop
		...
		for(int j=0; j<vec.size()-1; j++){
            ...
            int gap = 0;
			if(t.second < nodeNum[n]-1-stopNode[n].size() && s.second > stopNode[n].size()){
				gap = node[n][t.second].getTime()-node[n][s.second].getTime();
				if(gap==0)
					gap=1;
				//cout<<gap<<endl;
			}
...
void gurobi(){
	...
	for(int i=0; i<trackN; i++){
		for(int j=0; j<arc[i].size(); j++){
			...
			name.clear();
			v<<"ArcVisited"<<i<<"_"<<j;;
			var[v.str()] = model->addVar(0, 1, arc[i][j].getWeight(), GRB_BINARY, v.str());
	...
	for(int i=0; i<crossArc.size(); i++){
			...
			name.clear();
			v<<"CrossArcVisited"<<i;
			var[v.str()] = model->addVar(0, 1, crossArc[i].getWeight(), GRB_BINARY, v.str());
			//model->addConstr(var[name.str()]>= 1,"");
	}
	model -> update();
	// flow balance
	...
	for(int i=0; i<trackN; i++){
		for(int j=0; j<arc[i].size(); j++){
			total += arc[i][j].getWeight();
			stringstream ss;
			ss<<"ArcVisited"<<i<<"_"<<j;
			if(var[ss.str()].get(GRB_DoubleAttr_X)==1){
				cover += arc[i][j].getWeight();
			}
		}
	}
	cout<<"%%: "<<cover/total<<endl;
}
