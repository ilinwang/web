 /*
    calculate optimal patrol routings to minimize the maximum affected passengers 
	caused by single attack that compromises a train route segment
...
vector<Arc> crossArc;
multimap<double,int,greater<double> > sortWeight; // first->weight,second->index
vector<int> sortArc;
//Node **node;
//Arc **arc;
//Arc *crossArc;
...
int main(){
   ...
   /* for(int i=0; i<trackN; i++){
    ...
    for(multimap<double,int,greater<double> >::iterator it=sortWeight.begin();it!=sortWeight.end();it++){
	        sortArc.push_back(it->second);
    }
    bool flag = 0;
    int position=0;
    int head = 0;
    int tail = sortWeight.size();
    if((head+tail)%2==0){
        position = (head+tail)/2;
    }
    else{
        position = (head+tail+1)/2;
    }
    cout<<"head: "<<head<<" tail: "<<tail<<" position: "<<position<<endl;
    while(flag==0){
        gurobi(flag, head, tail, position);
    }

   /* for(multimap<double,int>::iterator it=sortWeight.begin(); it != sortWeight.end(); it++){
			cout<<"weight: "<< it->first <<" index: " << it->second <<endl;// save node index
		}*/
   ...
   //cout<<"nodeNum: "<<nodeNum[n]<<endl;
        ...
        if(Count < stopName[n].size()){
            ...
            for(int i=0; i<crossNode.size(); i++){
                if(node[n][Count1].getstopName() == crossNode[i]){
                    ...
                    sortWeight.insert(pair<double,int> (0,whichArc));
                    density = 1;
                    break;
                }
            }
    ...
    for(int i=0; i<stopNode[n].size();i++){ // for each stop
        ...
        for(int j=0; j<vec.size()-1; j++){
            ...
            for(int i=0; i<crossNode.size(); i++){
                if(node[n][s.second].getstopName() == crossNode[i]){
                    arc[n][whichArc].setArc(s, t, 10*(rand()%4+9));
                    sortWeight.insert(pair<double,int> (arc[n][whichArc].getWeight(),whichArc));
                    density = 1;
                    break;
                }
            }
            if(density==0){
                arc[n][whichArc].setArc(s, t, 10*(rand()%4+6));
                sortWeight.insert(pair<double,int> (arc[n][whichArc].getWeight(),whichArc));
            }
    ...
    for(int i=0; i<trainNode[n].size();i++){ // for each stop
        ...
        for(int j=0; j<vec.size()-1; j++){
            ...
            for(int i=0; i<crossNode.size(); i++){
                if(node[n][s.second].getstopName() == crossNode[i]){
                    arc[n][whichArc].setArc(s, t, 10*(rand()%2+3));
                    sortWeight.insert(pair<double,int> (arc[n][whichArc].getWeight(),whichArc));
                    density = 1;
                    break;
                }
            ...
            if(density==0){
                arc[n][whichArc].setArc(s, t, 10*(rand()%2+2));
                sortWeight.insert(pair<double,int> (arc[n][whichArc].getWeight(),whichArc));
            }
            NodeTo[n][vec.at(j)].push_back(whichArc);
            NodeFrom[n][vec.at(j+1)].push_back(whichArc);
...
void gurobi(bool& flag, int& head, int& tail, int& position){
    ...
    for(int i=0; i<trackN; i++){
        for(int j=0; j<arc[i].size(); j++){
            stringstream name,v;
            name<<"Arc"<<i<<"_"<<j;
            var[name.str()] = model->addVar(0, INT_MAX, 0, GRB_INTEGER, name.str());
            name.clear();
            v<<"ArcVisited"<<i<<"_"<<j;;
            var[v.str()] = model->addVar(0, 1, 0, GRB_BINARY, v.str());
            //model->addConstr(var[name.str()]>= 1,"");
        }
    ...
    model->addConstr(start == guard,"source");
    model->addConstr(end == guard,"sink");
    ...
    for(int i=0; i<trackN; i++){
        for(int j=0; j<arc[i].size(); j++){
            ...
            c<<"Arc"<<i<<"_"<<j;
            v<<"ArcVisited"<<i<<"_"<<j;
            model -> addConstr( var[c.str()] >= INT_MAX*(var[v.str()]-1) + 0.001 , v.str()); //force if sa(i) doesn't visited, then sa_visited = 0
            model -> addConstr( var[c.str()] <= INT_MAX*var[v.str()] , v.str());
            //model->addConstr(var[name.str()]>= 1,"");
        }
    }
   /* for(int i=0; i<crossArc.size(); i++){
    ...
    for(int i=0; i<trackN; i++){
        for(int j=0; j<=position; j++){
	        stringstream c,v;
	        v<<"ArcVisited"<<i<<"_"<<sortArc[j];
	        model -> addConstr(var[v.str()] == 1,"");
	        j++;
	    }
        ...
        if(head == tail-1){
    		...
    		for(int i=0; i<trackN; i++){
        		...
        		for(int j=0; j<arc[i].size(); j++){
		            ...
		            c<<"Arc"<<i<<"_"<<j;
		            v<<"ArcVisited"<<i<<"_"<<j;
		            cout<<var[v.str()].get(GRB_DoubleAttr_X)<<" ";
