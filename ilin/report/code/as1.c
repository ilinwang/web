...
{
  int i, temp_node, path_nodes, curr_arc;
...
dest_node=high_node;
  ...
  path[0]=temp_node;
  path_nodes=0;
  printf("the path from %d to %d is  ", root_node, dest_node);
  ...
  for(i=path_nodes-2;i>0;i--) /* i=the 3rd node to the last 2nd node in the 
      ...
      if (arc_list[curr_arc].flag==1) /* block for arcs with flag-ON */
	{
	  ...
	  while(curr_arc>0) /* loop to scan for lower virtual networks */
	    {
	      curr_arc=curr_arc-org_nodes;
		...
		arc_list[curr_arc].capy++;
	    }
	  curr_arc=node_list[path[i]].pred_arc; /* resume arc pointer */
	  while(curr_arc<=arcs) /* loop to scan for upper virtual networks */
	    {
	      curr_arc=curr_arc+org_nodes;
	      if (arc_list[curr_arc].flag==1)
		{
...
printf(" ->%d COST=[%d]", path[i], node_list[path[i]].dist);
