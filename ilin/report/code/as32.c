...
printf("---------------------\n");
      ...
      for (i=0;i<=new_n_node;i++)    /* renew all nodes data for new demand */
	{
	  ...
	  star_node[i].heap=-1;
	  star_node[i].n_arc=star_node[i].org_n_arc;
      ...
      /*print_node();*/
		...
		break;
	      ...
	      heap[0]=heap[h];        /* replace the top heap with the bottom
					 one */
	      /*printf("--2-- heap[%d]=%d \n", h,heap[h]);*/
	      if (heap[0]==0)        /* 0 should never be in heap at all */
		{
		   print_heap(h); print_network();
		   exit(0);
		 }
	      star_node[heap[0]].heap=0; /* now the new top heap has an index
					    0 */
	      heap[h]=-1;             /* replace the original bottom one with 
					 -1 */
