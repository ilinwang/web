...
void initialize()
{
  ...
  if ((fppath=fopen("path.dat", "w"))== NULL)  /* open path data file */
    {
      fprintf(stderr, " Error opening path.dat\n");
      exit(1);
    }
  if ((fparcpath=fopen("arcpath.dat", "w"))== NULL)/* open arc_path data file*/
    {
      fprintf(stderr, " Error opening arcpath.dat\n");
      exit(1);
    }
  if ((fpvarc=fopen("varc.dat", "w"))== NULL)  /* open arc data file */
    {
      fprintf(stderr, " Error opening varc.dat\n");
      exit(1);
    }
  if ((fpvnode=fopen("vnode.dat", "w"))== NULL)  /* open node data file */
    {
      fprintf(stderr, " Error opening vnode.dat\n");
      exit(1);
    }
  if ((fpeq=fopen("eq.dat", "w"))== NULL)  /* open equipment data file */
    {
      fprintf(stderr, " Error opening eq.dat\n");
      exit(1);
    }
  ...
  for (j=2;j<=8;j++) /* loop to duplicate the Virtual Network */
      ...
      for (i=0;i<org_n_arc;i++)
	{
	  ...
	  star_arc[curr_arc].current_path=0;
	}
    }
...
int memory_allocation()
{
  ...
  for(i=0, ptr_arc=star_arc; i<=new_n_arc; i++, ptr_arc++)	
    /* Allocate the space for path list for each arc(in VN)
    !!Note!! now, I limit at most 100000 unit can be passed on an arc
      in the Virtual Network, I may change this later to be dynammically
      memory allocation */
    { 
      if ((ptr_arc->path_list=(int *)malloc(100000*sizeof(int)))==NULL)
	{
	/*   Output error and return failure if failed to allocate */
	  printf("Cannot allocate memory, job terminated\n");
	  return(0);
   ...
   for(i=0, ptr_node=star_node; i<=new_n_node; i++, ptr_node++)	
     /* Allocate the space for adjacency arc list */
    {
      if ((ptr_node->path_out_list=(int *)malloc(100000*sizeof(int)))==NULL)
	{
	  /* Output error and return failure if failed to allocate */
	  printf("Cannot allocate memory, job terminated\n");
	  return(0);
	}
      if ((ptr_node->path_in_list=(int *)malloc(100000*sizeof(int)))==NULL)
	{
	  /* Output error and return failure if failed to allocate */
	  printf("Cannot allocate memory, job terminated\n");
	  return(0);
	}
      if ((ptr_node->path_trans_list=(int *)malloc(100000*sizeof(int)))==NULL)
	{
	  /* Output error and return failure if failed to allocate */
	  printf("Cannot allocate memory, job terminated\n");
	  return(0);
...
/* DIJKSTRA(j, h):  one step of dijkstra.
   This function is to scan all the outgoing arcs from node j, mallocate
	               update heap, so that the top heap has smallest distance
...
void update_network()
{
  ...
  fprintf(fppath, "I=%d d=%d u=%d W=%d #=%d => %d(%d) ",pathIndex, demand, 
	  (unit+1), star_node[path[i]].wavelength, path_nodes-1, path[i], 
	  star_node[path[i]].org_no );

  (star_node[path[i]].path_out_list)[(star_node[path[i]].n_path_out)++]=pathIndex;
  ...
  for(i=path_nodes-2;i>0;i--) 
    ...
    */
      ...
      else  /* block for arcs with flag-OFF */
	{
      ...
      printf("-> %d(%d) ", path[i], star_node[path[i]].org_no);     
      fprintf(fppath, "%d(%d) ",path[i], star_node[path[i]].org_no);

      (star_arc[star_node[path[i]].pred_arc].path_list)[(star_arc[star_node[path[i]].pred_arc].current_path)++]=pathIndex;

      (star_node[path[i]].path_trans_list)[(star_node[path[i]].n_path_trans)++]=pathIndex;
    }
  /* after the above loop, the "last trans path" is in fact the "out path",
     so, I have to decrease no. of "trans path" by 1, and record this path to
     be the last "in path" for this node */
  star_node[path[i+1]].n_path_trans--;
  (star_node[path[i+1]].path_in_list)[(star_node[path[i+1]].n_path_in)++]=pathIndex;

  /*printf("%d %d \n",star_node[path[i+1]].n_path_in, pathIndex);*/
...
/*======================== print_arc_path() =============================*/
/* This is just used for checking the path in the arcs, write in "arcpath.dat"
-------------------------------------------------------------------------*/
void print_arc_path()
{
  int i, j;
  ...
  fprintf(fparcpath, "Index of Unit in the Arcs :\n");

  for(i=0, ptr_arc=star_arc;i<new_n_arc-16;i++, ptr_arc++)
  {
    fprintf(fparcpath, "ArcIndex: %d(%d->%d) #=%d => ", i, 
	    star_node[star_arc[i].from].org_no,
	    star_node[star_arc[i].to].org_no, star_arc[i].current_path);
    for (j=0; j<star_arc[i].current_path; j++)
      fprintf(fparcpath, "%d ", star_arc[i].path_list[j]); 
    fprintf(fparcpath,"\n");
  }
...
/*======================== print_varc() =============================*/
/* This is just used for checking the path in the arcs, write in "varc.dat"
-------------------------------------------------------------------------*/
void print_varc()
{
  int i, j;
  ...
  fprintf(fpvarc, "Information about Arcs in the Virtual Network :\n");

  for(i=0, ptr_arc=star_arc;i<new_n_arc-16;i++, ptr_arc++)
  {
    fprintf(fpvarc, "ArcIndex: %d(%d) [%d(%d)->%d(%d)] C=%d U=%d W=%d #P=%d : ", i, 
	    star_arc[i].org_no,
	    star_arc[i].from,star_node[star_arc[i].from].org_no,
	    star_arc[i].to,star_node[star_arc[i].to].org_no, 
	    star_arc[i].cost, star_arc[i].u, star_arc[i].wavelength,
	    star_arc[i].current_path);
    for (j=0; j<star_arc[i].current_path; j++)
      fprintf(fpvarc, "%d ", star_arc[i].path_list[j]); 
    fprintf(fpvarc,"\n");
  }
...
/*======================== print_vnode() =============================*/
/* This is just used for checking the path in the arcs, write in "vnode.dat"
-------------------------------------------------------------------------*/
void print_vnode()
{
  int i, j;
  ...
  fprintf(fpvnode, "Information about Nodes in the Virtual Network :\n");

  for(i=0, ptr_node=star_node;i<=new_n_node-2;i++, ptr_node++)
  {
    fprintf(fpvnode, "NodeIndex: %d(%d) wavelength=%d pred=%d(%d) pred_arc=%d(%d) n_arc=%d org_n_arc=%d : ", i, 
	    star_node[i].org_no, star_node[i].wavelength, star_node[i].pred,
	    star_node[star_node[i].pred].org_no, star_node[i].pred_arc,
	    star_arc[star_node[i].pred_arc].org_no, star_node[i].n_arc,
	    star_node[i].org_n_arc);
    for (j=0; j<star_node[i].current_arc; j++)
      fprintf(fpvnode, "%d ", star_node[i].arc_list[j]); 
    fprintf(fpvnode,"\n");
  }
...
/*======================== print_eq() =============================*/
/* This is just used for checking the path in the arcs, write in "eq.dat"
-------------------------------------------------------------------------*/
void print_eq()
{
  int i, j;
  ...
  fprintf(fpeq, "Information about Input/Transport/Output path for a Node in the Virtual Network :\n");

  for(i=0, ptr_node=star_node;i<=new_n_node-2;i++, ptr_node++)
  {
    fprintf(fpeq, "NodeIndex: %d(%d) wavelength=%d pred=%d(%d) pred_arc=%d(%d) n_arc=%d org_n_arc=%d : \n", i, 
	    star_node[i].org_no, star_node[i].wavelength, star_node[i].pred,
	    star_node[star_node[i].pred].org_no, star_node[i].pred_arc,
	    star_arc[star_node[i].pred_arc].org_no, star_node[i].n_arc,
	    star_node[i].org_n_arc);
    fprintf(fpeq, "    path for out only #out_path=%d ==> ",star_node[i].n_path_out);
    for (j=0; j<star_node[i].n_path_out; j++)
      fprintf(fpeq, "%d ", star_node[i].path_out_list[j]); 
    fprintf(fpeq,"\n");
    
    fprintf(fpeq, "    path for trans only #trans_path=%d ==> ",star_node[i].n_path_trans);
    for (j=0; j<star_node[i].n_path_trans; j++)
      fprintf(fpeq, "%d ", star_node[i].path_trans_list[j]); 
    fprintf(fpeq,"\n");
    
    fprintf(fpeq, "    path for in only #in_path=%d ==> ",star_node[i].n_path_in);
    for (j=0; j<star_node[i].n_path_in; j++)
      fprintf(fpeq, "%d ", star_node[i].path_in_list[j]); 
    fprintf(fpeq,"\n");
  }
...
/*======================== count_fiber() =============================*/
/* This is used for counting # fibers needed after the assignment
-------------------------------------------------------------------------*/
void count_fiber()
{
  int i, j, temp_arc_no=0, temp_wave_no=1;
  
  for (i=0; i<org_n_arc; i++)
    {
      temp_arc_no=i;
      printf("For link %d->%d, we need at least max{ %d ", star_arc[i].from, star_arc[i].to, star_arc[i].current_path);

      for (j=2; j<=8; j++)
	{
	  printf("%d ",star_arc[i+(j-1)*org_n_arc].current_path);

	  if (star_arc[temp_arc_no].current_path <
	      star_arc[i+(j-1)*org_n_arc].current_path)
	    {
	      temp_arc_no=i+(j-1)*org_n_arc;
	      temp_wave_no=j;
	    }
	}
      /* if ((star_arc[temp_arc_no].current_path%8)==0)
	 temp_no_fiber=star_arc[temp_arc_no].current_path/8;
	 else
	 temp_no_fiber=star_arc[temp_arc_no].current_path/8+1;
	 */
      printf("}=%d fibers (W=%d)\n", 
	     star_arc[temp_arc_no].current_path, temp_wave_no);
