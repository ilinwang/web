...
struct arc
{
  int from,
      ...
      u,
      flag,
      *path_list,  /* record which unit of demand passes through it */
      current_path,/* index for recording the index of path for 1 unit */
...
/*------ globol variables ------*/
FILE *fptopo, *fpmino, *fppath, *fparcpath, *fpvarc, *fpvnode, *fpeq;
...
int amount;
int org_root_node, org_dest_node;
int *heap, n_perm=0, root=0;
int demand=0, unit=0, pathIndex=0;
...
void initialize();
int short_path();
void update_network(), print_arc_path(), print_varc(), print_vnode(), print_eq();
void count_fiber();
