/***************************************************************************/
/* short_main.c main program for this shortest path problem */
/* 1.204, spring 1995 
...
void main()
{
  read_network(arc_list, &arcs, &high_node, &root_node, &dest_node, node_list);
  ...
  s_short(arc_list, arcs, high_node, head, root_node, dest_node, node_list);
  printf("=========================================\n");
...
/***************************************************************************/
/* fstar_lib.c -Functions to put arbitrary network into forward star format */
/* 1.204, spring 1995 
   This program is desigend by I-Lin Wang and Gary Wong.
   There are 9 files needed to be compiled:
   short_main.c: main program, contains procedures of this problem
   ...
   To compile this problem: 
       gcc short_main.c short.c fstar_lib.c sort_lib.c int_min_heap.c
***************************************************************************/
/*!!NOTE!!: We do not consider a node without any ajacent arcs!! 
  node no. start from 1; arc no. starts from 0 ; graph may disconnected, 
  or multi-arcs pair; Max # of nodes =2* Max # of arcs (because no single node
  in G */
...
void read_network(sort_t arc_list[], int *arcs, int *high_node,
			 int *root_node, int *dest_node, node_t node_list[])
{
  ...
  *high_node= 0;       /* Highest node number; assume node nos > 0 */
  ...
  if ((fp=fopen("network.dat", "r"))== NULL)
    {
      fprintf(stderr, " Error opening network.dat\n");
      exit(1);
    }
/*  fscanf(fp, "%d %d", &nodes, &arcs); */
  fscanf(fp, "%d %d", root_node, dest_node); 
  do
    {
      fscanf(fp, "%d %d %d %d", &arc_list[i].key, &arc_list[i].to,
      ...
      if (arc_list[i].key > *high_node)
	*high_node= arc_list[i].key;    
      if (arc_list[i].to > *high_node)
	*high_node= arc_list[i].to;
      i++;
    } while (!feof(fp));
  clearerr(fp);
  fclose(fp);
		  ...
		  node_t node_list[])
{
  ...
  for (i=1;i<arcs;i++)              /* loop for arcs, arc no.:0--arcs-1 */
    {
  ...
  for (i=0;i<=high_node;i++) /* loop for nodes, fill empty node */
...
/***************************************************************************/
/* short.c shortest path algorithm, label setting */
/* 1.204, spring 1995 
   This program is desigend by I-Lin Wang and Gary Wong.
   There are 9 files needed to be compiled:
   short_main.c: main program, contains procedures of this problem
...
/*       printf("from node no=%d with #arcs=%d\n",ptop_node->no,
	 ptop_node->arcs);
*/
	...
	/*   printf(" to_node= %d,  from_node_dist= %d,",to_node,
	     ptop_node->dist);
	     printf(" tmp_dist=%d to_node_curr_dist=%d \n",tmp_dist, 
	     node_list[to_node].dist);
...
void heapsort(info_t *heap[], int n)
{
  int k;
  info_t *t;
  for (k= n/2; k >= 1; k--)
